    ...
    <div class="connect-to-data">
      ...
      <div class="connected" v-if="route.path.includes('connected')">
        <div class="current-amount">
          ...
          <Message v-if="isLarger" severity="error">목표 금액보다 큰 가치는 안됩니다.</Message>
        </div>
      </div>
    </div>
    <CustomButton
...
onMounted(async () => {
  ...
  isLiked.value = wishlist.some((item) => {
    ...
    return product?.id === productInfoLike.id && product?.come_from === productInfoLike.come_from
  })
...
const connectToGoal = async () => {
  try {
    ...
    if (route.path.includes('connected')) {
      ...
      try {
        if (monthlyPay.value > targetAmount.value || currentAmount.value > targetAmount.value) {
          isLarger.value = true
          return
        }
